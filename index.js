(async () => {
  ...
  s4d.client.on("messageCreate", async (s4dmessage) => {
    if (
      String(s4dmessage.channel.name).includes(String("todo")) ||
      String(s4dmessage.channel.name).includes(String("checklist"))
    ) {
  ...
  s4d.client.on("messageReactionAdd", async (reaction, user) => {
    if (
      String(reaction.message.channel.name).includes(String("todo")) ||
      String(reaction.message.channel.name).includes(String("checklist"))
    ) {
      if (!user.bot) {
  ...
  s4d.client.on("messageReactionRemove", async (reaction, user) => {
    if (
      ...
      if (!user.bot) {
        tempMessage = String(reaction.message).replaceAll(
          "**COMPLETE: **",
          String(" "),
        );
